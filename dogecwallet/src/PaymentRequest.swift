...
struct PaymentRequest {
    ...
    func fetchRemoteRequest(completion: @escaping (PaymentRequest?) -> Void) {
        ...
        request.setValue("application/dogecash-paymentrequest", forHTTPHeaderField: "Accept")
        ...
        URLSession.shared.dataTask(with: request as URLRequest) { data, response, error in
            ...
            if response.mimeType?.lowercased() == "application/dogecash-paymentrequest" {
    ...
    static func requestString(withAddress: String, forAmount: UInt64) -> String {
        ...
        return "dogecash:\(withAddress)?amount=\(btcAmount)"
    }
